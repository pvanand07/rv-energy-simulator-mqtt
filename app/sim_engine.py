...
def _pattern_compressor(step: int, rng: random.Random) -> float:
    """Fridge compressor: ON 22/90 steps, then fan-only 0.06×"""
    phase = step % 90
    if phase < 22:
...
def _pattern_thermostat(step: int, rng: random.Random) -> float:
    """Water heater / AC: thermostat cycling with random period variation"""
    phase = (step + rng.randint(0, 10)) % 120
...
_CYCLE_FNS = {
    ...
    "constant":      lambda s, r: 1.0 + r.gauss(0, 0.02),
}
...
def run_day_simulation(
    ...
    for s in range(STEPS):
        ...
        sol_kw += rng.gauss(0, sol_kw * 0.05)   # ±5% cloud flutter
        sol_kw  = max(0, sol_kw)
